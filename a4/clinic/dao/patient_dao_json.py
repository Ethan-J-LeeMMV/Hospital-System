...
class PatientDAOJSON(PatientDAO):
    ...
    def retrieve_patients(self, search_string: str):
        """retrieves patients and makes sure it is not case sensitive"""
        retrieved_list = []
        for phn in self.patients: 
            patient = self.patients[phn]
            patient_name = patient.get_name().upper()
    ...
    def update_patient(self, key, patient):
        ...
        cur_patient = self.patients[key]
        cur_patient.name = patient.name
        cur_patient.birth_date = patient.birth_date
        cur_patient.phone = patient.phone
        cur_patient.email = patient.email
        cur_patient.address = patient.address
